...
vector<int> dijkstra(const vector<vector<Edge>> &graph, int source) {
  ...
  // Inicjalizacja etykiet odległości
  ...
  while (!heap.empty()) {
    // Usuwanie węzła o najmniejszej etykiecie odległości z kopca
    ...
    for (const Edge &edge : graph[u]) {
      ...
      // Aktualizacja etykiety odległości sąsiada, jeśli nowa ścieżka jest krótsza
...
vector<vector<Edge>> readGraph(const string &filename) {
  ...
  if (file.is_open()) {
    ...
    while (getline(file, line)) {
      ...
      // Wczytywanie liczby węzłów i krawędzi
      if (line == 'p') {
      ...
      if (line == 'a') {
        ...
        // Numeracja węzłów w pliku zaczyna się od 1, a w kodzie od 0
